...
export function categorizeError(error: any, context: { baseURL: string; modelId: string }): ModelValidationError {
    ...
    if (errorStr.includes('econnrefused') || errorStr.includes('fetch failed') || errorStr.includes('network')) {
        return {
            type: 'offline',
            severity: 'critical',
            message: `Cannot connect to llama.cpp at ${baseURL}. Ensure llama.cpp server is running and active.`,
            canRetry: true,
    ...
    if (errorStr.includes('timeout') || errorStr.includes('aborted')) {
        return {
            type: 'timeout',
            severity: 'medium',
            message: `Request to llama.cpp timed out. This might happen with large models or slow systems.`,
            canRetry: true,
    ...
    if (errorStr.includes('404') || errorStr.includes('not found')) {
        return {
            type: 'not_found',
            severity: 'high',
            message: `Model '${modelId}' not found. Check if model is loaded in llama.cpp server.`,
            canRetry: false,
            autoFixAvailable: false
        }
    }
    ...
    if (errorStr.includes('401') || errorStr.includes('403') || errorStr.includes('unauthorized')) {
        return {
            type: 'permission',
            severity: 'high',
            message: `Authentication or permission issue with llama.cpp. Check your configuration.`,
            canRetry: false,
            autoFixAvailable: false
        }
    }
...
export function generateAutoFixSuggestions(errorCategory: ModelValidationError): AutoFixSuggestion[] {
    ...
    switch (errorCategory.type) {
        case 'offline':
            suggestions.push({
                action: "Check if llama.cpp server is running",
                steps: [
            ...
            suggestions.push({
                ...
                steps: [
                    "1. Check if llama.cpp server is running on a different port",
        ...
        case 'not_found':
            suggestions.push({
                action: "Load model in llama.cpp",
                steps: [
                    "1. Start the llama.cpp server",
                    "2. Load your desired model",
                    "3. Ensure the model is available via API"
                ],
                automated: false
            })
            break
